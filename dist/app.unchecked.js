...
},{"d3-collection":2,"d3-dispatch":3,"d3-quadtree":5,"d3-timer":6}],5:[function(require,module,exports){
...
},{"incremental-convex-hull":8,"uniq":18}],8:[function(require,module,exports){
"use strict"
...
},{"bit-twiddle":1,"union-find":17}],15:[function(require,module,exports){
"use strict"
...
var constrain = function constrain(min, max, value) {
  if (value <= min) {
    return min;
  } else if (value >= max) {
    return max;
...
var createKernel = function createKernel(radius) {
  radius = ~~radius;
  var a = [];

  for (var y = -radius; y < radius + 1; y++) {
    for (var x = -radius; x < radius + 1; x++) {
      var len = Math.round(Math.sqrt(x * x + y * y));
      if (len <= radius) {
        a.push({
          x: x,
          y: y
...
var createSDevArray = function createSDevArray(luminanceArray, width, height, radius) {
  var a = new Float32Array(luminanceArray.length);
  var kernel = createKernel(radius);
  ...
  for (var index in a) {
    ...
    try {
      for (var _iterator = kernel[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
        var k = _step.value;
    ...
    try {
      for (var _iterator2 = kernel[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
        var _k = _step2.value;

        var _sx = x + _k.x;
        var _sy = y + _k.y;

        if (_sx < 0 || _sy < 0 || _sx >= width || _sy >= height) {
          continue;
        }

        var _value = luminanceArray[_sx + _sy * width];
        total += (_value - average) * (_value - average);
        count++;
      }
    } catch (err) {
    ...
    var value = Math.sqrt(total / count);
    a[index] = value;
  ...
  // normalize
  for (var _index in a) {
    a[_index] = (a[_index] - minValue) / (maxValue - minValue);
  }

  return a;
};

var createLuminanceArray = function createLuminanceArray(imageData) {
  var a = new Float32Array(imageData.data.length / 4);
...
var _class = function () {
  ...
  _createClass(_class, [{
    key: 'load',
    value: function load(image, size, radius) {
    ...
    key: 'getRGBA',
    value: function getRGBA(x, y) {
      x = constrain(0, this.imageData.width, ~~x);
      y = constrain(0, this.imageData.height, ~~y);

      var w = this.imageData.width;

      var R = this.imageData.data[(x + y * w) * 4 + 0] / 255;
      var G = this.imageData.data[(x + y * w) * 4 + 1] / 255;
      var B = this.imageData.data[(x + y * w) * 4 + 2] / 255;
      var A = this.imageData.data[(x + y * w) * 4 + 3] / 255;

      return [R, G, B, A];
    }
  }, {
    key: 'getLuminance',
    value: function getLuminance(x, y) {
...
var _delaunayTriangulate = require('delaunay-triangulate');

var _delaunayTriangulate2 = _interopRequireDefault(_delaunayTriangulate);
...
var V_ATTRACT = 0.001;
var V_THETA = -0.001;
var V_RADIUS_MIN = 0.01;
var V_RADIUS_MAX = 0.3;

var model = {};
var renderer = new _renderer2.default(model);
var picker = new _colorpicker2.default();
var image = new Image();

var imageScale = 1.0;

var rotate = function rotate(x, y, theta) {
...
var nodes = [];
var simulation = D3Force.forceSimulation(nodes).velocityDecay(0.2).force('collide', D3Force.forceCollide().radius(function (node, i) {
  var x = node.x * imageScale + picker.width / 2;
  var y = node.y * -imageScale + picker.height / 2;

  //const l = 1 - picker.getLuminance(x, y)
  var l = 1 - picker.getSDev(x, y);
  node.color = picker.getRGBA(x, y);

  var r = rotate(node.x, node.y, V_THETA);
...
var addNode = function addNode() {
  ...
  for (var i = 0; i < count; i++) {
    var v = rotate(Math.random() * 1.0, 0, Math.random() * Math.PI * 2);
    nodes.push({
      x: v[0],
      y: v[1]
    });
...
image.onload = function (ev) {
  ...
  renderer.start(function () {
    ...
    model.points = nodes.map(function (node) {
      return [node.x, node.y];
    });

    model.colors = nodes.map(function (node) {
      return node.color;
    });

    model.triangles = (0, _delaunayTriangulate2.default)(model.points);
    model.lines = [];

    for (var i in model.triangles) {
...
},{"./colorpicker":19,"./renderer":21,"d3-force":4,"delaunay-triangulate":7}],21:[function(require,module,exports){
'use strict';
...
var aspectVec2 = function aspectVec2(width, height) {
  if (width > height) {
...
var _class = function () {
  ...
  _createClass(_class, [{
    ...
    value: function render(ctx) {
      var aspect = aspectVec2(ctx.viewportWidth, ctx.viewportHeight);
      var zoom = 1 + Math.sin(ctx.time * 0.1) * 0.5;
